...
function loadDrinksFromLocalStorage() {
  ...
  if (savedProducts) {
    ...
    drinks.value = data.filter(product => product.category === 'drink');
  }
}
...
function addProduct(product: Product) {
  ...
  if (editingProductId.value !== undefined) {
    ...
    editingProductId.value = undefined; // Reset after editing
  } else {
...
function resetNewProduct() {
  newProduct.value = { id: 0, name: '', price: 0, discount: 0, category: 'drink', image: null };
}
</script>

<template>
  <div class="container mx-auto mt-10">
    <h1 class="text-2xl font-bold mb-6 text-center">Drink Products</h1>
    ...
    <table class="min-w-full border-collapse border border-gray-300 shadow-lg rounded-lg overflow-hidden">
      ...
      <tbody>
        <tr v-for="product in drinks" :key="product.id" class="hover:bg-gray-50 transition duration-200">
          ...
          <td class="border border-gray-300 px-4 py-2 flex space-x-2 justify-center text-gray-600">
            <input
              type="checkbox"
              class="form-checkbox"
